...
class Menu_page(QWidget):
    ...
    def retranslateUi_Menu(self):
        ...
        self.Rules_Button.clicked.connect(self.changer.rules)
        self.Play_Button.setText(_translate("MainWindow", "Новая игра"))
        self.Play_Button.clicked.connect(self.changer.game)
...
class GameWindow(QWidget):
    ...
    def restart_game(self):
        ...
        if len(self.button_clicked_list) != 0:
            for i in range(len(self.button_clicked_list)):
                self.button_clicked_list[i].setEnabled(True)
        self.button_clicked_list.clear()
    ...
    def guess_letter_one(self):  # Работа первого бота
        ...
        if possible_words:
            ...
            elif len(self.random_word) > 6 and len(self.random_word) <= 8 and len(list(set(self.random_word))) - len(
                    self.guessed_letters_in_word) <= 2:
                random_word = random.choice(possible_words)
                letter = random.choice(random_word)
            elif len(self.random_word) > 8 and len(list(set(self.random_word))) - len(
                    self.guessed_letters_in_word) <= 3:
                random_word = random.choice(possible_words)
                letter = random.choice(random_word)
    ...
    def player_win(self):
        if "_" not in self.hidden_word_display and self.result == 1:
            QTimer.singleShot(3500, self.changer.win)

    def player_lose(self):
        if "_" not in self.hidden_word_display and self.result == 0:
            self.label_player.clear()
            self.label_host.clear()
            QTimer.singleShot(3500, self.changer.lose)
    ...
    def show_modal(self):
        ...
        if response == QMessageBox.No:
            self.input_locked_bot = True
            # Создаем экземпляры окон
            self.changer.to_the_main_menu()


class Rules(QWidget):
    def __init__(self, changer):
        super().__init__()  # окно правил
        self.changer = changer
    ...
    def retranslateUi_Rules(self):
        ...
        self.main_menu.clicked.connect(self.changer.to_the_main_menu)
...
class Win(QWidget):
    ...
    def retranslateUi_win(self):
        ...
        self.pushButton.clicked.connect(self.changer.game)
        self.pushButton_2.setText(_translate("MainWindow", "Главное меню"))
        self.pushButton_2.clicked.connect(self.changer.to_the_main_menu)
...
class changer_page(QStackedWidget):
    def __init__(self, parent=None):
        ...
        self.addWidget(self.Rules)
        self.addWidget(self.Win)
        self.addWidget(self.Lose)
...
if __name__ == "__main__":
    ...
    changer = changer_page()
    MainWindow.setCentralWidget(changer)
